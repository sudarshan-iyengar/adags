...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint, debug_from,
    ...
    try:
        schedule = ast.literal_eval(opt.resolution_schedule)
        if not isinstance(schedule, dict): schedule = {0: 1.0}
    except:
        schedule = {0: 1.0}
    unique_scales = list(set(schedule.values()))
    # Init scene with ALL required scales
    scene = Scene(dataset, gaussians, num_pts=num_pts, num_pts_ratio=num_pts_ratio, time_duration=time_duration, resolution_scales=unique_scales)
    scene.opt = opt
    ...
    valid_milestones = [m for m in schedule.keys() if m <= first_iter]
    current_scale = schedule[max(valid_milestones)] if valid_milestones else unique_scales[0]
    print(f"\n[INITIALIZING] Starting training at scale {current_scale}x")
    training_dataset = scene.getTrainCameras(scale=current_scale)
    training_dataloader = DataLoader(training_dataset, batch_size=batch_size, shuffle=True,
                                     num_workers=0 if dataset.dataloader else 0,
                                     collate_fn=identity_collate, drop_last=True)

    dataloader_iterator = iter(training_dataloader)
    ...
    while iteration < opt.iterations + 1:
        iteration+=1
        # 1. Check for Curriculum Swap
        if iteration in schedule and iteration != first_iter:
            current_scale = schedule[iteration]
            print(f"\n[ITER {iteration}] Swapping curriculum scale to {current_scale}x")
            training_dataset = scene.getTrainCameras(scale=current_scale)
            training_dataloader = DataLoader(training_dataset, batch_size=batch_size, shuffle=True,
