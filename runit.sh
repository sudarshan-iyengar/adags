...
for SCENE in "${SCENES[@]}"; do
  for ENTRY in "${CONFIGS[@]}"; do
    ...
    RUN_TAG="Scaffold_Motion_Priors_${CFG_NAME}"
    ...
    TRAIN_JOBID=$(
      sbatch --parsable \
        ...
        --export=ALL,SCENE="$SCENE",RUN_TAG="$RUN_TAG",RUN_ID="$RUN_ID",RUN_LABEL="$CFG_NAME",DATASET_ROOT="$WORK/proj_adags/data/n3v",CONFIG="$CFG_PATH",WANDB_PROJECT="$WANDB_PROJECT",WANDB_ENTITY="$WANDB_ENTITY",WANDB_GROUP="$WANDB_GROUP",WANDB_MODE="$WANDB_MODE" \
    ...
    EVAL_JOBID=$(
      sbatch --parsable \
        ...
        --export=ALL,SCENE="$SCENE",RUN_TAG="$RUN_TAG",RUN_ID="$RUN_ID",RUN_LABEL="$CFG_NAME",DATASET_ROOT="$WORK/proj_adags/data/n3v",CKPT_ITER="$CKPT_ITER",CONFIG="$CFG_PATH",WANDB_PROJECT="$WANDB_PROJECT",WANDB_ENTITY="$WANDB_ENTITY",WANDB_GROUP="$WANDB_GROUP",WANDB_MODE="$WANDB_MODE" \
