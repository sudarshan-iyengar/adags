...
usage() {
  cat <<'EOF'
Usage:
  scripts/submit_lora_mask_mechanism_screen.sh [--dry-run] [--train-only] [--include-lowroi]

Submit the LoRA mask-supervision mechanism screen:
  - fixed_budget_lora_route0_filemask_600k
  - fixed_budget_lora_route0_filemask_residual_600k

By default, the lower-loss candidate is not submitted. Add --include-lowroi or
set MECHANISM_CANDIDATES explicitly after Candidate A shows static/dynamic
loss-pressure tradeoff evidence.
...
Options:
  ...
  --include-lowroi Include fixed_budget_lora_route0_filemask_lowroi_600k.
  -h, --help       Show this help.

Environment overrides:
  SCENES="cut_roasted_beef flame_steak sear_steak"
  MECHANISM_CANDIDATES="fixed_budget_lora_route0_filemask_600k fixed_budget_lora_route0_filemask_residual_600k"
  CKPT_ITER=6000
  TRAIN_TIME=05:30:00
  EVAL_TIME=00:50:00
  CPUS_PER_TASK=8
  ...
  WANDB_GROUP=lora_mask_mechanism_20260619
EOF
}
...
if [[ -n "${MECHANISM_CANDIDATES:-}" ]]; then
  CANDIDATES="$MECHANISM_CANDIDATES"
else
  CANDIDATES="fixed_budget_lora_route0_filemask_600k fixed_budget_lora_route0_filemask_residual_600k"
  if [[ "$INCLUDE_LOWROI" == "1" ]]; then
    CANDIDATES="$CANDIDATES fixed_budget_lora_route0_filemask_lowroi_600k"
  fi
fi
...
WANDB_GROUP="${WANDB_GROUP:-lora_mask_mechanism_20260619}"
...
candidate_method_family() {
  case "$1" in
    fixed_budget_lora_route0_filemask_600k)
      ...
      ;;
    fixed_budget_lora_route0_filemask_residual_600k)
      echo "lora_filemask_residual"
      ;;
    fixed_budget_lora_route0_filemask_lowroi_600k)
      echo "lora_filemask_lowroi"
      ;;
    *)
      echo "ERROR: unknown mechanism candidate: $1" >&2
      return 1
      ;;
  esac
}
...
candidate_tag() {
  case "$1" in
    fixed_budget_lora_route0_filemask_600k)
      echo "candidate:filemask"
      ;;
    fixed_budget_lora_route0_filemask_residual_600k)
      echo "candidate:filemask_residual"
      ;;
    fixed_budget_lora_route0_filemask_lowroi_600k)
      echo "candidate:filemask_lowroi"
      ;;
    *)
      echo "ERROR: unknown mechanism candidate: $1" >&2
      return 1
...
manifest="$PROJECT_ROOT/exp_index/lora_mask_mechanism_screen_${timestamp}.tsv"
...
for candidate in $CANDIDATES; do
  ...
  for scene in $SCENES; do
    ...
    wandb_extra_tags="phase:lora_mask_mechanism screen:mask_supervision baseline:fixed_budget_lora_route0_600k comparator:fixed_budget_lora_route0_dyn_600k ${candidate_extra_tag} method:${method_family} budget:600k no_external_sync"
    ...
    train_cmd=(
      ...
      --export=ALL,ADAGS_REPO_DIR="$REPO_ROOT",ADAGS_PROJECT_ROOT="$PROJECT_ROOT",SCENE="$scene",RUN_TAG="lora_mask_mechanism",RUN_ID="$run_id",RUN_LABEL="$candidate",DATASET_ROOT="$DATASET_ROOT",CONFIG="$cfg_path",WANDB_PROJECT="$WANDB_PROJECT",WANDB_ENTITY="$WANDB_ENTITY",WANDB_GROUP="$WANDB_GROUP",WANDB_MODE="$WANDB_MODE",WANDB_EXTRA_TAGS="$wandb_extra_tags",EXPERIMENT_NAME="lora_mask_mechanism",METHOD_FAMILY="$method_family",BUDGET_LABEL="600k"
    ...
    if [[ "$WITH_EVAL" == "1" ]]; then
      ...
      eval_cmd=(
        ...
        --export=ALL,ADAGS_REPO_DIR="$REPO_ROOT",ADAGS_PROJECT_ROOT="$PROJECT_ROOT",SCENE="$scene",RUN_TAG="lora_mask_mechanism_eval",RUN_ID="$run_id",RUN_DIR="$run_dir",RUN_LABEL="$candidate",DATASET_ROOT="$DATASET_ROOT",CKPT_ITER="$CKPT_ITER",CONFIG="$cfg_path",WANDB_PROJECT="$WANDB_PROJECT",WANDB_ENTITY="$WANDB_ENTITY",WANDB_GROUP="$WANDB_GROUP",WANDB_MODE="$WANDB_MODE",WANDB_EXTRA_TAGS="$wandb_extra_tags eval_ckpt:${CKPT_ITER}",EXPERIMENT_NAME="lora_mask_mechanism",METHOD_FAMILY="$method_family",BUDGET_LABEL="600k"
