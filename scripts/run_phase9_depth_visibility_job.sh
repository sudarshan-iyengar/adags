...
elif [[ "$ACTION" == "da3-conformance" || "$ACTION" == "produce-da3" || "$ACTION" == "fast-visibility-pilot" || "$ACTION" == "anchor-abstention-pilot" || "$ACTION" == "cut-opportunity-mining" ]]; then
...
export TORCH_EXTENSIONS_DIR="${PHASE9_TORCH_EXTENSIONS_DIR:-$PROJECT_ROOT/build/torch_extensions_jobs/$SLURM_JOB_ID}"
