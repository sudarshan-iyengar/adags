    ...
    for scene, group in grouped.items():
        for metric in metrics:
            ...
            if not vals:
                ...
                result[scene][metric] = max(vals) if METRIC_INFO[metric]["higher"] else min(vals)
    return result
...
def best_values_overall(avg_rows, metrics):
    ...
    for metric in metrics:
        ...
        if not vals:
            ...
            result[metric] = max(vals) if METRIC_INFO[metric]["higher"] else min(vals)
    return result
...
def sanitize_piece(text: str) -> str:
    ...
    return text.strip("_")[:120] if text else ""
...
def build_descriptor(args):
    ...
    if args.methods:
        parts.append("methods__" + "__".join(sanitize_piece(x) for x in args.methods))
    if args.method_contains:
        parts.append("methodcontains__" + "__".join(sanitize_piece(x) for x in args.method_contains))
    if args.method_regex:
        parts.append("methodregex__" + sanitize_piece(args.method_regex))
    if args.exclude_methods:
        parts.append("exclude_methods__" + "__".join(sanitize_piece(x) for x in args.exclude_methods))
    if args.exclude_method_contains:
        parts.append("exclude_methodcontains__" + "__".join(sanitize_piece(x) for x in args.exclude_method_contains))
    if args.scenes and args.scenes != SCENES:
        parts.append("scenes__" + "__".join(sanitize_piece(x) for x in args.scenes))
    if args.run_contains:
        parts.append("runcontains__" + "__".join(sanitize_piece(x) for x in args.run_contains))
    if args.run_regex:
        parts.append("runregex__" + sanitize_piece(args.run_regex))

    if not parts:
        return "all_methods_all_scenes"

    descriptor = "__".join(parts)
    return descriptor[:220]


def write_outputs(output_dir: Path, detailed_latex: str, avg_latex: str, meta: dict):
    output_dir.mkdir(parents=True, exist_ok=True)

    (output_dir / "detailed.tex").write_text(detailed_latex)
    (output_dir / "average.tex").write_text(avg_latex)
...
def parse_args():
    ...
    parser.add_argument(
        "--output-root",
        type=str,
        default=os.path.expandvars("$WORK/proj_adags/results_tables"),
    )
    parser.add_argument(
        "--output-name",
        type=str,
        default=None,
        help="Optional fixed subfolder name under output-root. If omitted, a readable name is auto-generated.",
    )
...
def main():
    ...
    descriptor = args.output_name if args.output_name else build_descriptor(args)
    output_dir = Path(args.output_root) / descriptor

    meta = {
        "base_path": args.base_path,
        "output_dir": str(output_dir),
        "methods_discovered": methods_on_disk,
        "filters": {
            "methods": args.methods,
            "method_contains": args.method_contains,
            "method_regex": args.method_regex,
        ...
        "num_rows": len(rows),
        "num_methods": len({r["method"] for r in rows}),
        "num_scenes": len({r["scene"] for r in rows}),
        "detailed_caption": args.detailed_caption,
        "detailed_label": args.detailed_label,
        "avg_caption": args.avg_caption,
        "avg_label": args.avg_label,
    }

    write_outputs(output_dir, detailed_latex, avg_latex, meta)

    print(f"\nWrote outputs to: {output_dir}")
    print(f"  {output_dir / 'detailed.tex'}")
    print(f"  {output_dir / 'average.tex'}")
