...
Environment overrides:
  ...
  JOB_PREFIX=event_candidate_refine
  TRAIN_EXPERIMENT_NAME=event_candidate_refine
  EVAL_EXPERIMENT_NAME=event_candidate_refine_eval
  METHOD_FAMILY=M1_candidate_local_refine
...
JOB_PREFIX="${JOB_PREFIX:-event_candidate_refine}"
TRAIN_EXPERIMENT_NAME="${TRAIN_EXPERIMENT_NAME:-event_candidate_refine}"
EVAL_EXPERIMENT_NAME="${EVAL_EXPERIMENT_NAME:-event_candidate_refine_eval}"
METHOD_FAMILY_NAME="${METHOD_FAMILY:-M1_candidate_local_refine}"
TRAIN_EXPERIMENT_TAG="${TRAIN_EXPERIMENT_TAG:-experiment:event_candidate_refine}"
EVAL_EXPERIMENT_TAG="${EVAL_EXPERIMENT_TAG:-experiment:event_candidate_refine_eval}"
METHOD_TAG="${METHOD_TAG:-method:M1_candidate_local_refine}"
...
submit_train() {
  ...
  for scene in $SCENES; do
    ...
    wandb_extra_tags="$TRAIN_EXPERIMENT_TAG $METHOD_TAG source_ckpt:${START_CKPT_ITER} refine_iter:${REFINE_ITER}"
    cmd=(
      sbatch --parsable
...
submit_eval() {
  ...
  tail -n +2 "$RUN_MANIFEST" | while IFS=$'\t' read -r _train_job _mode scene _source_run _source_ckpt run_dir config _stdout _stderr; do
    ...
    wandb_extra_tags="$EVAL_EXPERIMENT_TAG $METHOD_TAG eval_ckpt:${REFINE_ITER}"
    cmd=(
      sbatch --parsable
      -p "$PARTITION" -A "$ACCOUNT" --qos="$QOS"
      -N 1 --ntasks=1 --cpus-per-task="$CPUS_PER_TASK" --gres=gpu:1
