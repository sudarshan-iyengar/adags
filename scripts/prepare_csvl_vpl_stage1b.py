#!/usr/bin/env python3
"""Prepare an immutable exact-worktree binding for the one Stage-1B audit."""
...
from depth_visibility.association_audit import STAGE1_COMMIT
from depth_visibility.canonical import sha256_file


DEFAULT_MATRIX = REPO_ROOT / "configs/depth_visibility/phase9_csvl_vpl_stage1b_v1.json"
DEFAULT_RUN_ID = "P9-VPL-S1B-D01-CUT-S20260726"
...
def write_new_json(path: Path, payload: Any) -> None:
    if path.exists():
        raise RuntimeError(f"refusing to replace immutable Stage-1B authority: {path}")
...
def main() -> int:
    ...
    entry = matches[0]
    if entry["action"] != "audit-stage1b-association":
        raise RuntimeError("Stage-1B authority received the wrong action")
    commit = git("rev-parse", "HEAD")
    if commit != STAGE1_COMMIT:
        raise RuntimeError(f"Stage-1 commit context changed: {commit}")
    ...
    if terminal.exists():
        raise RuntimeError(f"Stage-1B terminal already exists: {terminal}")
    for output in entry["expected_outputs"]:
        path = Path(expand(output["path"]))
        if output["schema"] != "phase9-terminal-manifest-v1" and path.exists():
            raise RuntimeError(f"immutable Stage-1B output already exists: {path}")
    ...
    if any(path.exists() for path in (implementation_path, command_path, execution_path)):
        raise RuntimeError("Stage-1B authority already exists; inspect it instead of replacing it")
    ...
    source_paths = sorted((REPO_ROOT / "depth_visibility").glob("*.py")) + [
        ...
        REPO_ROOT / "scripts/prepare_csvl_vpl_stage1b.py",
        REPO_ROOT / "configs/depth_visibility/csvl_vpl_stage1_v1.json",
        REPO_ROOT / "configs/depth_visibility/csvl_vpl_stage1b_v1.json",
        matrix_path,
    ...
    for path in source_paths:
        ...
        seen.add(relative)
        source_files.append({
            "path": relative, "bytes": resolved.stat().st_size,
            "sha256": sha256_file(resolved),
        })
    implementation = {
        "schema_version": "phase9-stage1b-exact-worktree-implementation-v1",
        "run_id": args.run_id,
        "binding_mode": "exact_worktree_files_no_commit_claim",
        "branch": git("rev-parse", "--abbrev-ref", "HEAD"),
        "commit": commit,
        "base_stage1_commit": STAGE1_COMMIT,
        ...
        "interpretation": "Stage-1B exact bytes are bound without claiming a clean worktree; unrelated pre-existing changes remain outside the source-file binding.",
    }
    ...
    command_registry = {
        "schema_version": "phase9-stage1b-command-registry-v1",
        "run_id": args.run_id,
        "base_stage1_commit": STAGE1_COMMIT,
    ...
    if base_sha != entry["configuration"]["base_config_sha256"]:
        raise RuntimeError("Stage-1B base config hash differs from the run matrix")
    merged_identity = {
