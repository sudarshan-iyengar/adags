...
from scene.cameras import cameraList_from_camInfosVideo2
...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], num_pts=100_000, num_pts_ratio=1.0, time_duration=None):
        ...
        if self.from3dgs:
            self.gaussians.create_from_3dgs(self.from3dgs, self.cameras_extent)
            
        elif args.loaded_pth:
    ...
    def getTestCameras(self, scale=1.0):
        # dataset = CameraDataset(self.test_cameras[scale].copy(), self.white_background)
        # return DataLoader(dataset, batch_size=1, shuffle=False, num_workers=0, collate_fn=lambda x: x)
