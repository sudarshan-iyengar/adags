...
| R029 train | Matched 6000-to-6400 continuation control | `8ebb53d` | `48935431`, `48935450`, `48935478` | `refine-logs/route0_continue_6400_train_jobs_20260708_152429.tsv`, `configs/n3v/route0_continue_6400_control.yaml` | TRAIN COMPLETE / EVAL BLOCKED | Train jobs completed with `ExitCode=0:0` and wrote all three `chkpnt6400.pth` files. Eval submission attempts were blocked by Slurm account/partition acceptance: default `boost_usr_prod/euhpc_d21_034/boost_qos_lprod` returned invalid account/partition, environment-loaded retry returned the same, and alternate-QoS submit hung before job IDs. No eval/scoring verdict yet. |
| R030 train | Frozen-crop support, Gaussian-only micro-densification | `8ebb53d` | `48935580`, `48935581`, `48935583` | `refine-logs/oracle_crop_support_micro_densify_train_jobs_20260708_153750_manual.tsv`, `configs/n3v/oracle_crop_support_micro_densify_6400.yaml` | TRAIN COMPLETE / EVAL BLOCKED | Diagnostic only; uses frozen R009 crops as support but no GT compositing. Train jobs completed with `ExitCode=0:0` and wrote all three `chkpnt6400.pth` files. Duplicate cut jobs from timed-out submissions (`48935560`, `48935682`) are excluded. Eval submission is blocked by the same Slurm account/partition issue as R029. |
