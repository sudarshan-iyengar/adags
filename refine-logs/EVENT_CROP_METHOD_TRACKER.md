...
| A2 | Motion-supported candidate discovery dry-run | pending local commit | local smoke | `refine-logs/hide_reveal_poc/local_smoke/nonoracle_candidates/out_motion_supported/` | SMOKE PASS | Revised detector multiplies dynamic/static-delta/flicker evidence by motion-mask support when masks are available; local smoke still selects the synthetic moving-square support. |
