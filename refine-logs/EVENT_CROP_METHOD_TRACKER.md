...
| weak support diagnostic | R031 DA3 depth_occlusion_support | n/a | n/a | n/a | n/a | `refine-logs/depth_occlusion_support/r031_support_overlap_full/support_overlap_summary.json` |
| weak support diagnostic | R032 DA3 high-recall sparse support | n/a | n/a | n/a | n/a | `refine-logs/depth_occlusion_support/r032_support_overlap_highrecall/support_overlap_summary.json` |
| weak support diagnostic | R033 DA3 high-recall tile-fill support | n/a | n/a | n/a | n/a | `refine-logs/depth_occlusion_support/r033_support_overlap_tilefill/support_overlap_summary.json` |
...
- R029 matched continuation worsened route0, so R027's tiny positive movement was not generic extra training.
- R030 oracle-support diagnostic worsened route0 and passed `0/5` route0 PSNR+L1 windows, so support alignment alone does not rescue the current posthoc micro-densification recipe.
- R031-R033 show DA3 setup/inference/support-generation PASS, but current DA3 support localization is weak: R031 mean crop coverage `0.000030`, R032 `0.002846`, R033 `0.001253`, versus R020 high-recall boxes `0.491371`. Do not treat DA3 support as a positive method result.
...
| R027 train | M2 event-boundary micro-densification train resumes | `efca178e7fd5fef49aa84c762350dfa8cbdd36d8` | `48873219`, `48873220`, `48873221` | `refine-logs/event_boundary_micro_densify_train_jobs_20260707_234937.tsv`, `logs/event_boundary_micro_densify_train_*_488732*.{out,err}` | TRAIN COMPLETE | Jobs completed with `ExitCode=0:0` in 18-20 minutes and wrote `chkpnt6400.pth` plus `point_cloud/iteration_6400/point_cloud.ply` for all three scenes. Logs show per-job extension roots, active ROI/static-exclusion terms, and final point counts below the 625k cap. |
...
| R031 predeclare/tooling | DA3 depth-occlusion support | `14f3c83` | n/a | `scripts/run_depth_occlusion_support.py`, `scripts/submit_depth_occlusion_support.sh`, `refine-logs/DEPTH_OCCLUSION_EVENT_SUPPORT_PLAN.md` | COMPLETE | Added non-oracle DA3 frame manifest, depth inference, support builder, and Slurm wrapper. Primary model predeclared as `depth-anything/DA3NESTED-GIANT-LARGE-1.1`; frozen R009 crops remain posthoc audit only. |
| R031 setup | DA3 repo/env/model setup | `14f3c83` | setup logs `da3_setup_20260709_200815.out`, repairs `da3_env_repair_*`, `da3_xformers_repair_*` | HPC: `$WORK/proj_adags/repo/depth-anything-3`, `$WORK/proj_adags/models/depth-anything/DA3NESTED-GIANT-LARGE-1.1` | PASS | DA3 repo cloned at `41736238f5bced4debf3f2a12375d2466874866d`; 6.8GB model snapshot cached locally. Initial env pulled incompatible local Torch and missed `addict`/`torchvision`; repaired to use ADAGS Torch/Torchvision with local `numpy<2` and `xformers==0.0.28.post3`. |
| R031 smoke | DA3 six-frame smoke and support builder | `14f3c83` | prepare `49029452`; infer fail `49029467`; infer retry `49029847`; support `49029955` | `refine-logs/depth_occlusion_support/r031_smoke_*` | PASS after repair | First infer smoke failed on `ModuleNotFoundError: addict`. After env repair, retry wrote 6 depth sidecars; support job wrote 6 masks with validation OK and non-oracle flags false. |
| R031 full depth | DA3 full `cam00` depth sidecars | `14f3c83` | prepare `49030039`; infer `49030185` | `refine-logs/depth_occlusion_support/r031_da3_frame_manifest.json`, `r031_da3_depth_full/da3_depth_manifest.json` | PASS | Prepared 900 frames across three scenes; inference wrote 900 depth sidecars with `written=900`, `skipped_existing=0`, elapsed `00:05:09`, max RSS about `7047772K`; no compute-node network dependency. |
| R031 support+audit | Sparse DA3 support | `14f3c83` | support `49030546` | `refine-logs/depth_occlusion_support/r031_depth_support_full/`, `r031_support_overlap_full/` | WEAK / NOT CLAIMED | Validation OK, 83 support frames, non-oracle flags false. Posthoc diagnostic overlap: mean support-frame fraction `0.0625`, mean crop coverage `0.000030`. This barely exceeds R026's zero crop coverage and is not enough to justify a positive support claim. |
| R032 support sensitivity | High-recall sparse DA3 support | `14f3c83` | support `49031389` | `refine-logs/depth_occlusion_support/r032_depth_support_highrecall/`, `r032_support_overlap_highrecall/` | WEAK / NOT CLAIMED | Predeclared sensitivity to over-pruning: 360 comps/scene, lower min score, larger tile/dilate. Validation OK, 355 support frames. Overlap improved temporal hit rate but mean crop coverage only `0.002846`, far below R020 `0.491371`. |
| R033 support sensitivity | High-recall tile-fill DA3 support | `dac940a` | support `49032424` | `refine-logs/depth_occlusion_support/r033_depth_support_highrecall_tilefill/`, `r033_support_overlap_tilefill/` | WEAK / NOT CLAIMED | Added `--fill-component-tiles` and reran high-recall support with 3% cap. Validation OK, 408 support frames, but mean crop coverage `0.001253`; tile-fill did not rescue localization. |
